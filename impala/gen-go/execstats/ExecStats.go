...
func (p TExecState) String() string {
	switch p {
	case TExecState_REGISTERED:
		return "REGISTERED"
	case TExecState_PLANNING:
		return "PLANNING"
	case TExecState_QUEUED:
		return "QUEUED"
	case TExecState_RUNNING:
		return "RUNNING"
	case TExecState_FINISHED:
		return "FINISHED"
	case TExecState_CANCELLED:
		return "CANCELLED"
	case TExecState_FAILED:
		return "FAILED"
	}
	return "<UNSET>"
}
...
func TExecStateFromString(s string) (TExecState, error) {
	switch s {
	case "REGISTERED":
		return TExecState_REGISTERED, nil
	case "PLANNING":
		return TExecState_PLANNING, nil
	case "QUEUED":
		return TExecState_QUEUED, nil
	case "RUNNING":
		return TExecState_RUNNING, nil
	case "FINISHED":
		return TExecState_FINISHED, nil
	case "CANCELLED":
		return TExecState_CANCELLED, nil
	case "FAILED":
		return TExecState_FAILED, nil
	}
...
func (p *TPlanNodeExecSummary) Read(iprot thrift.TProtocol) error {
	...
	if !issetFragmentID {
		return thrift.NewTProtocolExceptionWithType(thrift.INVALID_DATA, fmt.Errorf("Required field FragmentID is not set"))
	}
	if !issetLabel {
...
func (p *TPlanNodeExecSummary) ReadField8(iprot thrift.TProtocol) error {
	...
	for i := 0; i < size; i++ {
		...
		p.IsActive = append(p.IsActive, _elem1)
	}
	if err := iprot.ReadListEnd(); err != nil {
		return thrift.PrependError("error reading list end: ", err)
	}
...
func (p *TPlanNodeExecSummary) writeField2(oprot thrift.TProtocol) (err error) {
	if err := oprot.WriteFieldBegin("fragment_id", thrift.I32, 2); err != nil {
		return thrift.PrependError(fmt.Sprintf("%T write field begin error 2:fragment_id: ", p), err)
	}
	if err := oprot.WriteI32(int32(p.FragmentID)); err != nil {
		return thrift.PrependError(fmt.Sprintf("%T.fragment_id (2) field write error: ", p), err)
	}
	if err := oprot.WriteFieldEnd(); err != nil {
		return thrift.PrependError(fmt.Sprintf("%T write field end error 2:fragment_id: ", p), err)
	}
	return err
}
...
func (p *TPlanNodeExecSummary) writeField8(oprot thrift.TProtocol) (err error) {
	if p.IsSetIsActive() {
		if err := oprot.WriteFieldBegin("is_active", thrift.LIST, 8); err != nil {
			return thrift.PrependError(fmt.Sprintf("%T write field begin error 8:is_active: ", p), err)
		}
		if err := oprot.WriteListBegin(thrift.BOOL, len(p.IsActive)); err != nil {
			return thrift.PrependError("error writing list begin: ", err)
		}
		for _, v := range p.IsActive {
		...
		if err := oprot.WriteFieldEnd(); err != nil {
			return thrift.PrependError(fmt.Sprintf("%T write field end error 8:is_active: ", p), err)
		}
	}
	return err
}

func (p *TPlanNodeExecSummary) writeField9(oprot thrift.TProtocol) (err error) {
	if p.IsSetIsBroadcast() {
		if err := oprot.WriteFieldBegin("is_broadcast", thrift.BOOL, 9); err != nil {
			return thrift.PrependError(fmt.Sprintf("%T write field begin error 9:is_broadcast: ", p), err)
		}
		if err := oprot.WriteBool(bool(*p.IsBroadcast)); err != nil {
			return thrift.PrependError(fmt.Sprintf("%T.is_broadcast (9) field write error: ", p), err)
		}
		if err := oprot.WriteFieldEnd(); err != nil {
			return thrift.PrependError(fmt.Sprintf("%T write field end error 9:is_broadcast: ", p), err)
		}
	}
	return err
}
